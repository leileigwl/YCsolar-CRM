...
docker build -t leileigwl/ycsolar-frontend:1.1.0 -f docker/Dockerfile.frontend .

# 构建后端镜像
Write-Host "构建后端镜像..." -ForegroundColor Green
docker build -t leileigwl/ycsolar-backend:1.1.0 -f docker/Dockerfile.backend .

# 构建数据库镜像
Write-Host "构建数据库镜像..." -ForegroundColor Green
docker build -t leileigwl/ycsolar-db:1.1.0 -f docker/Dockerfile.db .
